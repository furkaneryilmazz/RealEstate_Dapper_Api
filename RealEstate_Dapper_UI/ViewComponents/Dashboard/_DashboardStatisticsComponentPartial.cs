...
namespace RealEstate_Dapper_UI.ViewComponents.Dashboard
{
    public class _DashboardStatisticsComponentPartial : ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var responseMessage1 = await client1.GetAsync("https://localhost:7101/api/Statistics/ProductCount");
            var jsonData1 = await responseMessage1.Content.ReadAsStringAsync();
            ViewBag.ProductCount = jsonData1;
            #endregion

            #region ist2-EnBaşarılıPersonel
            var client2 = _httpClientFactory.CreateClient(); // Create a new HttpClient instance
            var responseMessage2 = await client2.GetAsync("https://localhost:7101/api/Statistics/EmployeeNameByMaxProductCount");
            var jsonData2 = await responseMessage2.Content.ReadAsStringAsync();
            ViewBag.EmployeeNameByMaxProductCount = jsonData2;
            #endregion


            #region ist3-İlandakiŞehirSayısı
            var client3 = _httpClientFactory.CreateClient(); // Create a new HttpClient instance
            var responseMessage3 = await client3.GetAsync("https://localhost:7101/api/Statistics/DifferentCityCount");
