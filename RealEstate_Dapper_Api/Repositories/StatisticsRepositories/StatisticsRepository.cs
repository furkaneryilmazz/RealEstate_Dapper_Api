...
namespace RealEstate_Dapper_Api.Repositories.StatisticsRepositories
{
    public class StatisticsRepository : IStatisticsRepository
    {
        ...
        public int ApartmanCount()
        {
            string query = "Select Count(*) From Product where Tittle like '%Daire%'";
            using (var connection = _context.CreateConnection())
            {
                var values = connection.QueryFirstOrDefault<int>(query);
                return values;
        ...
        {
            string query = "Select Top(1) CategoryName, Count(*) From Product inner join Category On Product.ProductKategory=Category.CategoryID Group By CategoryName order by Count(*) Desc ";
            using (var connection = _context.CreateConnection())
            {
        ...
        {
            string query = "SELECT COUNT(DISTINCT City) AS 'Farkli_Sehir_Sayisi' FROM Product ";
            using (var connection = _context.CreateConnection())
            {
                var values = connection.QueryFirstOrDefault<int>(query);
                return values;
